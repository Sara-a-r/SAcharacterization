...
On the system acts a viscous friction force with a coefficient gamma and an external force.
...
#--------------------------Temporal evolution----------------------#
def evolution(evol_method, Nt_step, dt, physical_params, signal_params, F, ref, kp, ki, kd, file_name = None):
    ...
    #temporal evolution when the ext (control+external) force is applied
    # temporal evolution when the ext force is applied
    for Fi in F_signal:
        y_t = evol_method(y_t, A, B, Fi+Fc)  # step n+1
        v1.append(y_t[0])
        v2.append(y_t[1])
        v3.append(y_t[2])
        v4.append(y_t[3])
...
if __name__ == '__main__':
    ...
    M = [173, 165, 140, 118, 315]  # filter mass [Kg]  [M1, M2, M3, M4, M5]
    K = [702.63612454,  859.32565959, 2623.82233935, 1424.4203582,  1968.15021859]  # spring constant [N/m]  [K1, K2, K3, K4, K5]
    F0 = 0.5  # amplitude of the external force #Note : consider also the coeff in B
    w = 0.16  # f of the ext force
    ...
    control_params_PID = [ref, 400, 0, 400]    # ref, kp, ki, kd
    ...
    tt, v1_PID, v2_PID, v3_PID, v4_PID, v5_PID, x1_PID, x2_PID, x3_PID, x4_PID, x5_PID = evolution(*simulation_params,
                                                           physical_params,signal_params, F, *control_params_PID,
                                                           file_name=None)
    ...
    plt.plot(tt, x5_PID, linestyle='-', linewidth=1, marker='', color='darkmagenta', label='x5, mass M5')
    plt.legend()
    plt.tight_layout()
